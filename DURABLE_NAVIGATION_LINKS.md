# 🔗 Durable Website Navigation Links

**Copy these exact links into your Durable website navigation/menu**

---

## 📋 MAIN NAVIGATION (Top Menu)

```
Home
https://www.elevateconnectsdirectory.org/

Programs
https://www.elevateconnectsdirectory.org/programs

About
https://www.elevateconnectsdirectory.org/about

Partners
https://www.elevateconnectsdirectory.org/partners

For Employers
https://www.elevateconnectsdirectory.org/employers

Contact
https://www.elevateconnectsdirectory.org/contact
```

---

## 🎓 PROGRAMS DROPDOWN

```
All Programs
https://www.elevateconnectsdirectory.org/programs

Medical Assistant
https://www.elevateconnectsdirectory.org/programs/medical-assistant

HVAC Technician
https://www.elevateconnectsdirectory.org/programs/hvac

Barber Apprenticeship
https://www.elevateconnectsdirectory.org/programs/barber

CDL Training
https://www.elevateconnectsdirectory.org/programs/truck-driving

Building Tech
https://www.elevateconnectsdirectory.org/programs/building-tech

Workforce Readiness
https://www.elevateconnectsdirectory.org/programs/workforce-readiness
```

---

## 🚀 CALL-TO-ACTION BUTTONS

```
Join for Free / Apply Now
https://www.elevateconnectsdirectory.org/apply

Enroll Now
https://www.elevateconnectsdirectory.org/enroll

Get Started
https://www.elevateconnectsdirectory.org/apply

Talk to an Advisor
https://www.elevateconnectsdirectory.org/contact

Request Info
https://www.elevateconnectsdirectory.org/contact
```

---

## 👤 USER PORTALS (Login Links)

```
Student Login
https://www.elevateconnectsdirectory.org/student/dashboard

Instructor Login
https://www.elevateconnectsdirectory.org/instructor/dashboard

Admin Login
https://www.elevateconnectsdirectory.org/admin/dashboard

Program Holder Login
https://www.elevateconnectsdirectory.org/program-holder/dashboard

Employer Login
https://www.elevateconnectsdirectory.org/employer/dashboard

Sign Up
https://www.elevateconnectsdirectory.org/signup

Log In
https://www.elevateconnectsdirectory.org/login
```

---

## 📚 RESOURCES SECTION

```
Financial Aid
https://www.elevateconnectsdirectory.org/financial-aid

Success Stories
https://www.elevateconnectsdirectory.org/success-stories

FAQ
https://www.elevateconnectsdirectory.org/faq

Blog
https://www.elevateconnectsdirectory.org/blog

Careers
https://www.elevateconnectsdirectory.org/careers

Pricing
https://www.elevateconnectsdirectory.org/pricing
```

---

## 🏢 FOOTER LINKS

### Column 1: About

```
About Us
https://www.elevateconnectsdirectory.org/about

What We Offer
https://www.elevateconnectsdirectory.org/programs

Partners
https://www.elevateconnectsdirectory.org/partners

Careers
https://www.elevateconnectsdirectory.org/careers
```

### Column 2: Community

```
For Learners
https://www.elevateconnectsdirectory.org/student/dashboard

For Employers
https://www.elevateconnectsdirectory.org/employers

For Training Providers
https://www.elevateconnectsdirectory.org/program-holder/dashboard

Blog
https://www.elevateconnectsdirectory.org/blog
```

### Column 3: Support

```
Contact
https://www.elevateconnectsdirectory.org/contact

Help Center
https://www.elevateconnectsdirectory.org/faq

Financial Aid
https://www.elevateconnectsdirectory.org/financial-aid

Success Stories
https://www.elevateconnectsdirectory.org/success-stories
```

### Column 4: Legal

```
Privacy Policy
https://www.elevateconnectsdirectory.org/privacy-policy

Terms of Service
https://www.elevateconnectsdirectory.org/terms-of-service

Accessibility
https://www.elevateconnectsdirectory.org/accessibility
```

---

## 🎯 SPECIFIC LANDING PAGES

```
Workforce Enrollment
https://www.elevateconnectsdirectory.org/enroll/workforce

General Enrollment
https://www.elevateconnectsdirectory.org/enroll

Application Form
https://www.elevateconnectsdirectory.org/apply

Certificate Verification
https://www.elevateconnectsdirectory.org/verify
```

---

## 📱 SOCIAL MEDIA LINKS (Add your actual URLs)

```
Facebook
https://facebook.com/elevateforhumanity

Twitter
https://twitter.com/elevateforhumanity

LinkedIn
https://linkedin.com/company/elevateforhumanity

Instagram
https://instagram.com/elevateforhumanity

YouTube
https://youtube.com/@elevateforhumanity
```

---

## 🔧 HOW TO ADD TO DURABLE

### For Navigation Menu:

1. Go to Durable website editor
2. Click on Navigation/Menu
3. Add "Link" element
4. Paste the URL from above
5. Add link text (e.g., "Programs", "About", etc.)

### For Buttons:

1. Add a Button element
2. Click "Link" or "URL"
3. Paste the URL (e.g., `/apply`)
4. Style the button

### For Dropdown Menus:

1. Create parent menu item (e.g., "Programs")
2. Add child items underneath
3. Link each child to specific program page

---

## 📋 COPY-PASTE READY FORMAT

**For Durable's Link Field:**

```
/                           (Homepage)
/programs                   (All Programs)
/programs/medical-assistant (Medical Assistant)
/programs/hvac              (HVAC)
/programs/barber            (Barber)
/about                      (About)
/partners                   (Partners)
/employers                  (Employers)
/contact                    (Contact)
/apply                      (Apply/Enroll)
/login                      (Login)
/signup                     (Sign Up)
/faq                        (FAQ)
/blog                       (Blog)
/financial-aid              (Financial Aid)
/success-stories            (Success Stories)
/privacy-policy             (Privacy)
/terms-of-service           (Terms)
```

---

## 🎨 RECOMMENDED NAVIGATION STRUCTURE

```
Header:
├── Logo (links to /)
├── Programs (dropdown)
│   ├── All Programs
│   ├── Medical Assistant
│   ├── HVAC Technician
│   ├── Barber Apprenticeship
│   └── CDL Training
├── About
├── Partners
├── For Employers
├── Contact
└── Buttons:
    ├── Log In
    └── Join for Free (primary CTA)

Footer:
├── About Column
├── Community Column
├── Support Column
├── Legal Column
└── Social Media Icons
```

---

## ⚡ QUICK COPY BLOCKS

### Hero Section CTA:

```html
<a href="https://www.elevateconnectsdirectory.org/apply">Join for Free</a>
<a href="https://www.elevateconnectsdirectory.org/programs">Explore Programs</a>
```

### Program Cards:

```html
<a href="https://www.elevateconnectsdirectory.org/programs/medical-assistant"
  >Learn More →</a
>
<a href="https://www.elevateconnectsdirectory.org/programs/hvac"
  >Learn More →</a
>
<a href="https://www.elevateconnectsdirectory.org/programs/barber"
  >Learn More →</a
>
```

### Footer CTA:

```html
<a href="https://www.elevateconnectsdirectory.org/apply"
  >Start My Application</a
>
<a href="https://www.elevateconnectsdirectory.org/contact"
  >Talk to an Advisor</a
>
```

---

## 🔍 TESTING CHECKLIST

After adding links to Durable, test:

- ✅ All navigation links work
- ✅ Dropdown menus expand
- ✅ CTA buttons link correctly
- ✅ Footer links work
- ✅ Mobile menu works
- ✅ Links open in same tab (internal)
- ✅ Social links open in new tab (external)

---

## 💡 PRO TIPS

1. **Use relative URLs** in Durable if possible:
   - `/programs` instead of full URL
   - Durable will auto-add your domain

2. **Set link targets:**
   - Internal links: Same tab
   - External links: New tab (`target="_blank"`)

3. **Add tracking:**
   - Use UTM parameters for marketing links
   - Example: `/apply?utm_source=durable&utm_medium=nav`

4. **Mobile optimization:**
   - Test all links on mobile
   - Ensure dropdown menus work on touch

5. **Update regularly:**
   - Add new programs as they launch
   - Update seasonal CTAs
   - Keep social links current

---

**Need help adding these to Durable? Let me know which section you're working on!**
