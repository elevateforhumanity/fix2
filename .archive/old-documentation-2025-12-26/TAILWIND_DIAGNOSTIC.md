...
```css
@media (max-width: 768px) /* Targets < md */ @media (max-width: 640px) /* Targets < sm */ @media (max-width: 480px); /* Custom, not in Tailwind */
```
...
/* NEW */
.grid:not([class*='sm:grid-cols']):not([class*='md:grid-cols']):not(
    [class*='lg:grid-cols']
  ) {
  grid-template-columns: 1fr !important;
}
