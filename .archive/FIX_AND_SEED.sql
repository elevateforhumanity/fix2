...
ON CONFLICT (slug) DO UPDATE SET description = EXCLUDED.description, duration = EXCLUDED.duration, moderation_status = EXCLUDED.moderation_status, category = EXCLUDED.category, level = EXCLUDED.level, updated_at = NOW();

SELECT id, title, duration, moderation_status, category, level FROM courses WHERE moderation_status = 'approved' ORDER BY title;
