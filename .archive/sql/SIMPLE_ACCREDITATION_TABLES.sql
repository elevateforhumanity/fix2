-- ============================================
-- SIMPLE ACCREDITATION TABLES
-- Run this ONLY if you already have: profiles, enrollments, programs, courses tables
-- ============================================
...
CREATE TABLE IF NOT EXISTS student_records (
  ...
  UNIQUE(profile_id, enrollment_id)
);
...
CREATE TABLE IF NOT EXISTS attendance_records (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  UNIQUE(profile_id, enrollment_id, date)
);
...
CREATE INDEX IF NOT EXISTS idx_student_records_profile ON student_records(profile_id);
CREATE INDEX IF NOT EXISTS idx_student_records_enrollment ON student_records(enrollment_id);
...
CREATE INDEX IF NOT EXISTS idx_grades_profile ON grade_records(profile_id);
CREATE INDEX IF NOT EXISTS idx_grades_enrollment ON grade_records(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_grades_course ON grade_records(course_id);

CREATE INDEX IF NOT EXISTS idx_hours_profile ON hour_tracking(profile_id);
CREATE INDEX IF NOT EXISTS idx_hours_enrollment ON hour_tracking(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_hours_date ON hour_tracking(date DESC);

CREATE INDEX IF NOT EXISTS idx_sap_profile ON sap_records(profile_id);
CREATE INDEX IF NOT EXISTS idx_sap_enrollment ON sap_records(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_sap_date ON sap_records(check_date DESC);

CREATE INDEX IF NOT EXISTS idx_complaints_profile ON complaints(profile_id);
CREATE INDEX IF NOT EXISTS idx_complaints_status ON complaints(status);

CREATE INDEX IF NOT EXISTS idx_refunds_profile ON refunds(profile_id);
...
CREATE INDEX IF NOT EXISTS idx_ecr_profile ON ecr_snapshots(profile_id);
CREATE INDEX IF NOT EXISTS idx_ecr_enrollment ON ecr_snapshots(enrollment_id);
CREATE INDEX IF NOT EXISTS idx_ecr_date ON ecr_snapshots(snapshot_date DESC);

CREATE INDEX IF NOT EXISTS idx_welcome_profile ON welcome_packets(profile_id);
CREATE INDEX IF NOT EXISTS idx_welcome_enrollment ON welcome_packets(enrollment_id);

CREATE INDEX IF NOT EXISTS idx_onboarding_profile ON onboarding_checklist(profile_id);
CREATE INDEX IF NOT EXISTS idx_onboarding_enrollment ON onboarding_checklist(enrollment_id);

CREATE INDEX IF NOT EXISTS idx_emails_profile ON email_notifications(profile_id);
CREATE INDEX IF NOT EXISTS idx_emails_sent ON email_notifications(sent_at DESC);

CREATE INDEX IF NOT EXISTS idx_mou_profile ON mou_signatures(profile_id);
CREATE INDEX IF NOT EXISTS idx_mou_enrollment ON mou_signatures(enrollment_id);
...
BEGIN
  ...
  RAISE NOTICE '📊 Created 16 tables for compliance tracking';
  RAISE NOTICE '🎯 Next step: Run COPY_PASTE_TRIGGERS.sql';
END $$;
