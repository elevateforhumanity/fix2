  ...
  try {
    ...
    const result = await pool.query(
      ...
      [
        ...
        JSON.stringify(trainingPrograms || []),
        JSON.stringify(supportServices || []),
...
export async function updateIEP(req: AuthRequest, res: Response) {
  try {
    ...
    if (updates.currentSkills)
      updates.current_skills = JSON.stringify(updates.currentSkills);
    if (updates.skillGaps)
      updates.skill_gaps = JSON.stringify(updates.skillGaps);
    if (updates.barriers) updates.barriers = JSON.stringify(updates.barriers);
    if (updates.strengths)
      updates.strengths = JSON.stringify(updates.strengths);
    if (updates.trainingPrograms)
      updates.training_programs = JSON.stringify(updates.trainingPrograms);
    if (updates.supportServices)
      updates.support_services = JSON.stringify(updates.supportServices);
    ...
    if (result.rows.length === 0) {
      return res.status(404).json({
        success: false,
        error: { code: 'NOT_FOUND', message: 'IEP not found' },
      });
    }
