...
export async function getEmployerById(req: AuthRequest, res: Response) {
  try {
    ...
    const result = await pool.query('SELECT * FROM employers WHERE id = $1', [
      id,
    ]);
...
export async function updateJobPosting(req: AuthRequest, res: Response) {
  try {
    ...
    const fields = Object.keys(updates).filter(
      (k) => k !== 'id' && !k.includes('At') && k !== 'applications'
    );
    if (fields.length === 0) {
      return res.status(400).json({
        success: false,
...
export async function updateApplicationStatus(req: AuthRequest, res: Response) {
  try {
    ...
    const values = fields
      .filter((f) => f !== 'updated_at' && f !== 'offer_date')
...
export async function addRetentionCheck(req: AuthRequest, res: Response) {
  try {
    const { id } = req.params;
    const { quarter, employed, wage, hoursPerWeek, contactMethod, notes } =
      req.body;

    const placement = await pool.query(
      'SELECT retention_checks FROM placements WHERE id = $1',
      [id]
    );
    ...
    const retentionChecks = JSON.parse(
      placement.rows[0].retention_checks || '[]'
    );
    const newCheck = {
      id: `check_${Date.now()}`,
      checkDate: new Date(),
