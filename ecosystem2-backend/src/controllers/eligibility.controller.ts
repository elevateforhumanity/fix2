...
import {
  ...
  EligibilityCreateInput,
  EligibilityUpdateInput,
  EligibilityApprovalInput,
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Not authorized to view this eligibility record',
        },
      });
    }
  ...
  try {
    ...
    const result = await pool.query(
      ...
      [
        ...
        input.gender,
        input.ethnicity,
        JSON.stringify(input.race || []),
        input.isVeteran || false,
        input.veteranDocumentUrl,
        input.isDislocatedWorker || false,
        input.dislocatedWorkerDocumentUrl,
        input.layoffDate,
        input.isLowIncome || false,
        input.incomeDocumentUrl,
        input.householdSize,
        input.annualIncome,
        input.isYouth || false,
        input.hasDisability || false,
        input.disabilityDocumentUrl,
  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Cannot update approved/denied eligibility record',
        },
      });
    }
  ...
  try {
    ...
    if (req.user!.role !== 'admin' && req.user!.role !== 'case_manager') {
      return res.status(403).json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Only admins and case managers can approve eligibility',
        },
      });
    }
  ...
  try {
    if (req.user!.role !== 'admin' && req.user!.role !== 'case_manager') {
      return res.status(403).json({
        ...
        error: {
          code: 'FORBIDDEN',
          message: 'Only admins and case managers can view pending eligibility',
        },
      });
    }
