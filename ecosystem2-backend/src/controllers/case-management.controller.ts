...
export async function createCase(req: AuthRequest, res: Response) {
  try {
    ...
    if (intakeNotes) {
      ...
      await pool.query(`UPDATE case_management SET notes = $1 WHERE id = $2`, [
        JSON.stringify([note]),
        id,
      ]);
    }
...
export async function updateCase(req: AuthRequest, res: Response) {
  try {
    ...
    if (updates.accommodations)
      updates.accommodations = JSON.stringify(updates.accommodations);
...
export async function addCaseNote(req: AuthRequest, res: Response) {
  try {
    ...
    const caseRecord = await pool.query(
      'SELECT notes FROM case_management WHERE id = $1',
      [id]
    );
...
export async function addCaseActivity(req: AuthRequest, res: Response) {
  try {
    ...
    const caseRecord = await pool.query(
      'SELECT activities FROM case_management WHERE id = $1',
      [id]
    );
    if (caseRecord.rows.length === 0) {
      return res.status(404).json({
...
export async function addReferral(req: AuthRequest, res: Response) {
  try {
    ...
    const caseRecord = await pool.query(
      'SELECT referrals FROM case_management WHERE id = $1',
      [id]
    );
    if (caseRecord.rows.length === 0) {
      return res.status(404).json({
