-- LOCKDOWN PART 3: Add Day-1 Allow Policies (FINAL)
...
BEGIN
  ...
  -- Applications: Anyone can submit (contact form)
  SELECT EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'applications') INTO table_exists;
  IF table_exists THEN
    BEGIN
      EXECUTE 'CREATE POLICY "anyone_can_submit_application" ON public.applications FOR INSERT TO anon, authenticated WITH CHECK (true)';
    EXCEPTION WHEN duplicate_object THEN NULL;
    END;
  END IF;

  -- Enrollments: Users can read their own (if table has student_id or user_id)
  ...
  IF table_exists THEN
    BEGIN
      ...
      EXECUTE 'CREATE POLICY "users_read_own_enrollments" ON public.enrollments FOR SELECT TO authenticated USING (auth.uid() = student_id)';
    EXCEPTION 
      WHEN duplicate_object THEN NULL;
      WHEN undefined_column THEN
        -- If student_id doesn't exist, try user_id
        BEGIN
          EXECUTE 'CREATE POLICY "users_read_own_enrollments" ON public.enrollments FOR SELECT TO authenticated USING (auth.uid() = user_id)';
        EXCEPTION WHEN duplicate_object THEN NULL;
        END;
    END;
  END IF;
