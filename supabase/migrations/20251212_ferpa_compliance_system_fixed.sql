-- FERPA Compliance System Migration (FIXED)
-- Creates tables for FERPA training, certifications, and compliance tracking
-- This version removes problematic RLS policies that reference non-existent columns
...
DROP TRIGGER IF EXISTS update_ferpa_student_acknowledgments_updated_at ON ferpa_student_acknowledgments;
CREATE TRIGGER update_ferpa_student_acknowledgments_updated_at 
  BEFORE UPDATE ON ferpa_student_acknowledgments
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_ferpa_consent_forms_updated_at ON ferpa_consent_forms;
CREATE TRIGGER update_ferpa_consent_forms_updated_at 
  BEFORE UPDATE ON ferpa_consent_forms
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_ferpa_violation_reports_updated_at ON ferpa_violation_reports;
CREATE TRIGGER update_ferpa_violation_reports_updated_at 
  BEFORE UPDATE ON ferpa_violation_reports
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_data_sharing_agreements_updated_at ON data_sharing_agreements;
CREATE TRIGGER update_data_sharing_agreements_updated_at 
  BEFORE UPDATE ON data_sharing_agreements
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_ferpa_compliance_checklist_updated_at ON ferpa_compliance_checklist;
CREATE TRIGGER update_ferpa_compliance_checklist_updated_at 
  BEFORE UPDATE ON ferpa_compliance_checklist
  FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Row Level Security (RLS) Policies
-- Note: These policies assume a 'profiles' table exists with 'id' and 'role' columns
...
DROP POLICY IF EXISTS "Users can view their own training records" ON ferpa_training_records;
CREATE POLICY "Users can view their own training records"
  ON ferpa_training_records FOR SELECT
  USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can view all training records" ON ferpa_training_records;
...
DROP POLICY IF EXISTS "Users can insert their own training records" ON ferpa_training_records;
...
DROP POLICY IF EXISTS "Students can view their own acknowledgments" ON ferpa_student_acknowledgments;
CREATE POLICY "Students can view their own acknowledgments"
  ON ferpa_student_acknowledgments FOR SELECT
  USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can insert their own acknowledgments" ON ferpa_student_acknowledgments;
CREATE POLICY "Students can insert their own acknowledgments"
  ON ferpa_student_acknowledgments FOR INSERT
  WITH CHECK (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can update their own acknowledgments" ON ferpa_student_acknowledgments;
CREATE POLICY "Students can update their own acknowledgments"
  ON ferpa_student_acknowledgments FOR UPDATE
  USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Admins can view all acknowledgments" ON ferpa_student_acknowledgments;
...
DROP POLICY IF EXISTS "Admins can view disclosure log" ON ferpa_disclosure_log;
...
DROP POLICY IF EXISTS "Authorized users can insert disclosures" ON ferpa_disclosure_log;
...
DROP POLICY IF EXISTS "Students can view their own consent forms" ON ferpa_consent_forms;
CREATE POLICY "Students can view their own consent forms"
  ON ferpa_consent_forms FOR SELECT
  USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can insert their own consent forms" ON ferpa_consent_forms;
CREATE POLICY "Students can insert their own consent forms"
  ON ferpa_consent_forms FOR INSERT
  WITH CHECK (auth.uid() = student_id);

DROP POLICY IF EXISTS "Students can update their own consent forms" ON ferpa_consent_forms;
CREATE POLICY "Students can update their own consent forms"
  ON ferpa_consent_forms FOR UPDATE
  USING (auth.uid() = student_id);

DROP POLICY IF EXISTS "Admins can view all consent forms" ON ferpa_consent_forms;
...
DROP POLICY IF EXISTS "Admins can view access log" ON ferpa_access_log;
...
DROP POLICY IF EXISTS "System can insert access log" ON ferpa_access_log;
...
DROP POLICY IF EXISTS "Anyone can insert violation reports" ON ferpa_violation_reports;
CREATE POLICY "Anyone can insert violation reports"
  ON ferpa_violation_reports FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins can view violation reports" ON ferpa_violation_reports;
...
DROP POLICY IF EXISTS "Admins can update violation reports" ON ferpa_violation_reports;
...
DROP POLICY IF EXISTS "Admins can manage data sharing agreements" ON data_sharing_agreements;
...
DROP POLICY IF EXISTS "System can insert audit log" ON lms_security_audit_log;
CREATE POLICY "System can insert audit log"
  ON lms_security_audit_log FOR INSERT
  WITH CHECK (true);

DROP POLICY IF EXISTS "Admins can view audit log" ON lms_security_audit_log;
...
DROP POLICY IF EXISTS "Admins can manage compliance checklist" ON ferpa_compliance_checklist;
