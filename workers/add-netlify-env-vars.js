  ...
  async fetch(request, env) {
    ...
    if (!supabase_service_role_key) {
      return new Response(JSON.stringify({
        ...
        usage: 'POST with JSON body: { "supabase_service_role_key": "eyJhbGci..." }'
      }), {
        status: 400,
        headers: { 'Content-Type': 'application/json' }
      });
    ...
    return new Response(JSON.stringify({
      ...
      deploy_url: deploySuccess ? `https://app.netlify.com/sites/${SITE_ID}/deploys/${deployResult?.id}` : null,
      site_url: 'https://elevateconnectsdirectory.org',
      message: allSuccess && deploySuccess 
