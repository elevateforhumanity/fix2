...
export class AutopilotMetrics {
  ...
  private async handleHistory(url: URL, corsHeaders: any): Promise<Response> {
    ...
    const historicalChecks = this.checks.filter(
  ...
  private async handleTrends(url: URL, corsHeaders: any): Promise<Response> {
    ...
    const trends = Array.from(hourlyBuckets.entries()).map(([hour, checks]) => {
      const avgSuccessRate =
        checks.reduce((sum, c) => sum + c.overall.successRate, 0) /
        checks.length;
      const healthyCount = checks.filter(
        (c) => c.overall.status === 'healthy'
      ).length;
      const degradedCount = checks.filter(
        (c) => c.overall.status === 'degraded'
      ).length;
      const criticalCount = checks.filter(
        (c) => c.overall.status === 'critical'
      ).length;
  ...
  private async handleAlerts(corsHeaders: any): Promise<Response> {
    const alerts = this.checks.filter(
      (check) => check.alertSent || check.issueCreated
    );
  ...
  private calculateSummary(): MetricsSummary {
    ...
    const healthyCount = this.checks.filter(
      (c) => c.overall.status === 'healthy'
    ).length;
    const degradedCount = this.checks.filter(
      (c) => c.overall.status === 'degraded'
    ).length;
    const criticalCount = this.checks.filter(
      (c) => c.overall.status === 'critical'
    ).length;

    const avgSuccessRate =
      this.checks.reduce((sum, c) => sum + c.overall.successRate, 0) /
      this.checks.length;
    ...
    for (let i = 0; i < this.checks.length - 1; i++) {
      if (
        this.checks[i].overall.status === 'critical' &&
        this.checks[i + 1].overall.status === 'healthy'
      ) {
        const diff =
          new Date(this.checks[i + 1].timestamp).getTime() -
          new Date(this.checks[i].timestamp).getTime();
