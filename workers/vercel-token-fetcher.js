...
class VercelTokenFetcher {
  ...
  loadCredentials() {
    ...
    console.log(`   GitHub Token: ${creds.githubToken ? '✅ Found' : '❌ Not set'}`);
  ...
  async loginWithGitHub() {
    ...
    try {
      ...
      await this.page.waitForSelector('button[data-testid="login-with-github"]', { timeout: 5000 });
      ...
      if (this.page.url().includes('github.com')) {
        ...
        await this.page.waitForSelector('button[name="authorize"]', { timeout: 5000 });
  ...
  async fetchTokenFromDashboard() {
    ...
    try {
      ...
      await this.page.goto('https://vercel.com/account/tokens', { waitUntil: 'networkidle2' });
      ...
      await this.page.waitForSelector('button:has-text("Create")', { timeout: 5000 });
      await this.page.click('button:has-text("Create")');
      
      // Fill token details
      await this.page.waitForSelector('input[name="name"]', { timeout: 5000 });
      await this.page.type('input[name="name"]', `Autopilot Token ${Date.now()}`);
      ...
      // Get token value
      await this.page.waitForSelector('[data-testid="token-value"]', { timeout: 5000 });
  ...
  async fetchProjectInfo() {
    ...
    try {
      ...
      await this.page.goto('https://vercel.com/dashboard', { waitUntil: 'networkidle2' });
    ...
    for (const [key, value] of Object.entries(secrets)) {
      if (value && !value.includes('PLACEHOLDER')) {
        try {
          ...
          console.log(`  ⚠️  Could not upload ${key} - gh CLI not authenticated`);
        }
      }
    }
  ...
  async run() {
    try {
      ...
      if (this.tokens.VERCEL_TOKEN || this.tokens.VERCEL_PROJECT_ID) {
        ...
        console.log('📊 Results:');
        console.log(`   VERCEL_TOKEN: ${this.tokens.VERCEL_TOKEN ? '✅ Found' : '❌ Not found'}`);
        console.log(`   VERCEL_ORG_ID: ${this.tokens.VERCEL_ORG_ID ? '✅ Found' : '❌ Not found'}`);
        console.log(`   VERCEL_PROJECT_ID: ${this.tokens.VERCEL_PROJECT_ID ? '✅ Found' : '❌ Not found'}`);
      } else {
