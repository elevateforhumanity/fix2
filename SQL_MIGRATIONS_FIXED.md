# 🗄️ DATABASE MIGRATIONS - FIXED (COPY & PASTE)

**Run these in order in Supabase SQL Editor**

---

## MIGRATION 1: Onboarding Tutorials (FIXED)
...
CREATE INDEX idx_user_tutorials_tutorial_id ON user_tutorials(tutorial_id);
CREATE INDEX idx_user_tutorials_completed ON user_tutorials(completed);
...
DROP POLICY IF EXISTS "Admins can view all program holder students" ON program_holder_students;
CREATE POLICY "Admins can view all program holder students" ON program_holder_students FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
);

DROP POLICY IF EXISTS "Admins can manage program holder students" ON program_holder_students;
CREATE POLICY "Admins can manage program holder students" ON program_holder_students FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
);
```
...
-- RLS Policies
DROP POLICY IF EXISTS "Everyone can view active MOU templates" ON mou_templates;
CREATE POLICY "Everyone can view active MOU templates" ON mou_templates FOR SELECT USING (active = true);

DROP POLICY IF EXISTS "Admins can manage MOU templates" ON mou_templates;
CREATE POLICY "Admins can manage MOU templates" ON mou_templates FOR ALL USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
);

DROP POLICY IF EXISTS "Program holders can view their signatures" ON mou_signatures;
...
DROP POLICY IF EXISTS "Admins can view all signatures" ON mou_signatures;
CREATE POLICY "Admins can view all signatures" ON mou_signatures FOR SELECT USING (
  EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin')
);
```
...
-- RLS Policies
DROP POLICY IF EXISTS "Employees can view their own documents" ON hr_documents;
CREATE POLICY "Employees can view their own documents" ON hr_documents FOR SELECT USING (auth.uid() = employee_id);

DROP POLICY IF EXISTS "Admins can manage all HR documents" ON hr_documents;
...
Each one handles existing tables and won't fail if columns already exist!
