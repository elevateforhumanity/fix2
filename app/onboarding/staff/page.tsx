...
export default function StaffOnboarding() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    alert(
      'Application submitted! HR will contact you within 2-3 business days.'
    );
  };
      ...
      <header className="border-b border-white/10 bg-black">
        <div className="mx-auto max-w-4xl px-6 py-6">
          ...
          <p className="mt-2 text-slate-300">
            Join the Elevate For Humanity team
          </p>
        </div>
      </header>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      id_upload: e.target.files?.[0] || null,
                    })
                  }
                  className="w-full rounded-lg border border-white/20 bg-slate-800 px-4 py-2 text-white focus:border-orange-400 focus:outline-none"
                />
            ...
            <div className="space-y-3">
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  required
                  checked={formData.professional_boundaries}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      professional_boundaries: e.target.checked,
                    })
                  }
                  className="mt-1 w-4 h-4 rounded border-white/20 bg-slate-800 text-orange-500"
                />
                <span className="text-sm">
                  I will maintain professional boundaries with learners and
                  partners
                </span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  required
                  checked={formData.communication_standards}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      communication_standards: e.target.checked,
                    })
                  }
                  className="mt-1 w-4 h-4 rounded border-white/20 bg-slate-800 text-orange-500"
                />
                <span className="text-sm">
                  I will follow Elevate communication standards
                </span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  required
                  checked={formData.document_progress}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      document_progress: e.target.checked,
                    })
                  }
                  className="mt-1 w-4 h-4 rounded border-white/20 bg-slate-800 text-orange-500"
                />
                <span className="text-sm">
                  I will document attendance, progress, and coaching notes
                </span>
              </label>
              <label className="flex items-start gap-3 cursor-pointer">
                <input
                  type="checkbox"
                  required
                  checked={formData.report_incidents}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      report_incidents: e.target.checked,
                    })
                  }
                  className="mt-1 w-4 h-4 rounded border-white/20 bg-slate-800 text-orange-500"
                />
                <span className="text-sm">
                  I will report safety, behavioral, or compliance issues
                  immediately
                </span>
              </label>
            </div>
