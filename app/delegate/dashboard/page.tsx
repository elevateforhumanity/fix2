...
export default async function DelegateDashboard() {
  ...
  const processedStudents =
    students?.map((student) => {
      ...
      const daysSinceLogin = lastLogin
        ? Math.floor(
            (Date.now() - new Date(lastLogin.login_time).getTime()) /
              (1000 * 60 * 60 * 24)
          )
        : null;
      ...
      const course =
        activeEnrollment &&
        (Array.isArray(activeEnrollment.courses)
          ? activeEnrollment.courses[0]
  ...
  const avgProgress =
    totalStudents > 0
      ? Math.round(
          processedStudents.reduce((sum, s) => sum + s.progress, 0) /
            totalStudents
        )
      : 0;
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="elevate-nav">
        ...
        <nav className="flex gap-6 items-center">
          ...
          <Link
            href="/delegate/reports"
            className="text-gray-700 hover:text-red-600 font-medium"
          >
            Reports
          </Link>
          <Link
            href="/delegate/students"
            className="text-gray-700 hover:text-red-600 font-medium"
          >
            Students
          </Link>
      ...
      <section className="elevate-hero">
        <div className="elevate-hero-content">
          ...
          <p className="elevate-hero-subtitle">
            Monitor student progress, attendance, and compliance for WRG/WIOA
            programs
          </p>
        </div>
      </section>
        ...
        <div className="grid gap-4 md:grid-cols-4 mb-8">
          <div className="elevate-card">
            ...
            <p className="text-xs text-slate-400 mt-2">
              {delegate?.caseload_limit
                ? `of ${delegate.caseload_limit} max`
                : 'Assigned to you'}
            </p>
          </div>
          <div className="elevate-card">
          ...
          <div className="mb-8 p-4 bg-red-50 border border-red-200 rounded-lg flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-red-800">
                {atRiskStudents} student{atRiskStudents > 1 ? 's have' : ' has'}{' '}
                not logged in for 7+ days. Consider reaching out to check on
                their progress and address any barriers.
              </p>
            </div>
          </div>
        )}
              ...
              <tbody>
                {processedStudents.length > 0 ? (
                  processedStudents.map((student) => (
                    <tr
                      key={student.id}
                      className={student.isAtRisk ? 'bg-red-50' : ''}
                    >
                      <td>
                        <div>
                      ...
                      <td>
                        <span
                          className={`elevate-pill text-xs ${
                            student.fundingType === 'wioa'
                              ? 'elevate-pill--info'
                              : student.fundingType === 'wrg'
                                ? 'elevate-pill--success'
                                : student.fundingType === 'jri'
                      ...
                      <td>
                        {student.lastLogin ? (
                          <div>
                            ...
                            <div
                              className={`text-xs ${student.isAtRisk ? 'text-red-600 font-semibold' : 'text-gray-500'}`}
                            >
