...
export default async function CRMHubPage() {
  ...
  const [
    ...
    supabase.from('follow_up_reminders').select('*', { count: 'exact', head: true }).eq('status', 'pending'),
    supabase.from('appointments').select('*', { count: 'exact', head: true }).gte('appointment_date', new Date().toISOString()),
    supabase.from('email_campaigns').select('*').order('created_at', { ascending: false }).limit(5),
    supabase.from('deals').select('*').in('stage', ['prospecting', 'qualification', 'proposal', 'negotiation']).limit(10),
  ]);
    ...
    <main className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">Manage all customer relationships and campaigns</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <Link href="/admin/crm/contacts" aria-label="Link" className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-gray-900">{totalContacts || 0}</span>
            </div>
          ...
          <Link href="/admin/crm/leads" aria-label="Link" className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-gray-900">{activeLeads || 0}</span>
            </div>
          ...
          <Link href="/admin/crm/follow-ups" aria-label="Link" className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-gray-900">{pendingFollowUps || 0}</span>
            </div>
          ...
          <Link href="/admin/crm/appointments" aria-label="Link" className="bg-white rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-4">
              ...
              <span className="text-2xl font-bold text-gray-900">{scheduledMeetings || 0}</span>
            </div>
            <h3 className="text-sm font-semibold text-gray-600">Upcoming Meetings</h3>
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          >
            ...
            <p className="text-green-100 text-sm">Track new sales opportunity</p>
          </Link>
          ...
          >
            ...
            <p className="text-purple-100 text-sm">Book appointment with Zoom</p>
          </Link>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8">
            ...
            {recentCampaigns && recentCampaigns.length > 0 ? (
              <div className="space-y-4">
                {recentCampaigns.map((campaign) => (
                  <div key={campaign.id} className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg">
          ...
          <div className="bg-white rounded-xl shadow-sm p-6">
            ...
            {openDeals && openDeals.length > 0 ? (
              <div className="space-y-4">
                {openDeals.map((deal) => (
                  <div key={deal.id} className="flex items-start gap-4 p-4 border border-gray-200 rounded-lg">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <span className="text-lg font-bold text-gray-900">${Number(deal.amount).toLocaleString()}</span>
