...
export const metadata: Metadata = {
  ...
  title: 'Program Holder Acknowledgements | Admin',
  description: 'Manage program-holder-acknowledgements',
};
...
export default async function ProgramHolderAcknowledgementsPage() {
  ...
  const { data: items, count } = await supabase
    .from('program-holder-acknowledgements')
    .select('*', { count: 'exact' })
      ...
      <section className="bg-blue-700 text-white py-12">
        <div className="container mx-auto px-4">
          <div className="max-w-7xl mx-auto">
            <h1 className="text-4xl font-bold mb-4">Program Holder Acknowledgements</h1>
            <p className="text-xl text-blue-100">Manage program-holder-acknowledgements</p>
          </div>
        </div>
      </section>
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {items.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-sm">
                          <Link 
                            href={`/admin/program-holder-acknowledgements/${item.id}`}
