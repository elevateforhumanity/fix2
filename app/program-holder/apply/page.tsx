...
export default function ProgramHolderApplicationPage() {
  ...
  const programOptions = [
    'Barber Training', 'CNA', 'CDL', 'HVAC', 'Medical Assistant',
    'Esthetician', 'Building Maintenance', 'Business & Entrepreneurship', 'All Programs'
  ];
  ...
  function handleProgramToggle(program: string) {
    ...
    if (current.includes(program)) {
      setFormData({ ...formData, programsInterested: current.filter(p => p !== program) });
    } else {
        ...
        <section className="py-16">
          <div className="max-w-2xl mx-auto px-6 text-center">
            <div className="bg-white rounded-lg shadow-sm border p-8">
              ...
              <p className="text-lg text-slate-600 mb-6">Our team will review your application and contact you within 2-3 business days.</p>
              <button onClick={() => router.push('/')} className="px-8 py-3 bg-blue-700 text-white rounded-lg font-semibold hover:bg-blue-800">Return to Home</button>
            </div>
          </div>
        </section>
      </div>
        ...
        <div className="max-w-4xl mx-auto px-6">
          {error && (
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <div className="flex items-center gap-2"><AlertCircle className="text-red-600" size={20} /><p className="text-red-800 font-semibold">Error</p></div>
...
onChange={handleChange} required className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500" /></div>
            ...
            <div className="bg-white rounded-lg shadow-sm border p-8">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">Program Interest</h2>
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-3">Programs Interested In *</label>
...
rows={4} className="w-full px-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500" /></div>
            ...
            <div className="flex gap-4">
              <button type="submit" disabled={submitting || formData.programsInterested.length === 0} className="flex-1 px-8 py-4 bg-blue-700 text-white rounded-lg font-semibold hover:bg-blue-800 disabled:bg-slate-300 disabled:cursor-not-allowed text-lg">{submitting ? 'Submitting...' : 'Submit Application'}</button>
              <button type="button" onClick={() => router.back()} className="px-8 py-4 bg-white text-slate-900 border-2 border-slate-300 rounded-lg font-semibold hover:bg-slate-50 text-lg">Cancel</button>
            </div>
          </form>
        </div>
      </section>
