...
export default async function ProgramHolderDashboard() {
  ...
  const atRiskStudents =
    enrollments?.filter((e) => e.student_risk_status?.[0]?.status === 'at_risk')
      .length || 0;
  const needsActionStudents =
    enrollments?.filter(
      (e) => e.student_risk_status?.[0]?.status === 'needs_action'
    ).length || 0;
  ...
  const { data: upcomingDeadlines } = await supabase
    .from('student_requirements')
    .select(
      '*, enrollments!inner(program_id, profiles!enrollments_student_id_fkey(first_name, last_name))'
    )
    .in('enrollments.program_id', programIds)
                  ...
                  {enrollments.map((enrollment: any) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <div
                            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full ${statusBadge.bgColor}`}
                          >
                            {statusBadge.icon}
                            <span
                              className={`text-sm font-semibold ${statusBadge.color}`}
                            >
                              {statusBadge.text}
                            </span>
                          </div>
        ...
        <div className="bg-blue-50 rounded-lg shadow-sm p-6 border-2 border-blue-200">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-gray-700">
                Monthly reporting due in 5 days
              </span>
            </div>
          </div>
        </div>
