...
export default function CalendarPage() {
  ...
  const todayEvents = upcomingEvents.filter(
    (e) => getDaysUntil(e.date) === 'Today'
  );
  const thisWeekEvents = upcomingEvents.filter((e) => {
    const days = getDaysUntil(e.date);
    return (
      (days !== 'Today' && days !== 'Past' && !days.includes('In')) ||
      (days.includes('In') && parseInt(days.split(' ')[1]) <= 7)
    );
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="mb-6">
          <Tabs
            value={selectedView}
            onValueChange={(v) => setSelectedView(v as 'month' | 'list')}
          >
            <TabsList>
              ...
              <section>
                ...
                <div className="space-y-3">
                  {todayEvents.map((event) => (
                    <Card
                      key={event.id}
                      className={`border-l-4 ${getPriorityColor(event.priority)}`}
                    >
            ...
            <CardContent>
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm mt-2">
                  Use list view to see all your events
                </p>
              </div>
            </CardContent>
          </Card>
