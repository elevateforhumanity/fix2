  ...
  try {
    if (type === 'image') {
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({ error: error.error?.message || 'DALL-E API error' }, { status: 500 });
      }
      ...
      return NextResponse.json({ error: 'Invalid type. Use "image" or "content"' }, { status: 400 });
    }
  } catch (error: any) {
    console.error('AI Asset Generator error:', error);
