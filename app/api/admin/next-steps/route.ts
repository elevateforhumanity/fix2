...
export async function GET(req: Request) {
  ...
  const summary = {
    total: rows.length,
    funding_pending: rows.filter((item) => r.funding_status === 'pending')
      .length,
    funding_approved: rows.filter((item) => r.funding_status === 'approved')
      .length,
    funding_denied: rows.filter((item) => r.funding_status === 'denied')
      .length,
    appt_missing: rows.filter((item) => !r.workone_appointment_scheduled)
      .length,
    onboarding_missing: rows.filter(
      (item) => !r.efh_onboarding_call_completed
    ).length,
    start_missing: rows.filter((item) => !r.program_start_confirmed).length,
  };
