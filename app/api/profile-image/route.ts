...
export async function POST(req:Request){
 ...
 if(!(file instanceof File)||!['student','host-shop'].includes(kind))return NextResponse.json({error:'Image and profile type are required'},{status:400});
 ...
 const bucket=kind==='host-shop'?'images':'profile-photos';
 const path=kind==='host-shop'?'host-shops/'+user.id+'/hero.'+ext:'apprentices/'+user.id+'/profile.'+ext;
