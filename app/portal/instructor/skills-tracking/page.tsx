...
const BARBER_SKILLS = [
  ...
  { id: 'scissor-over-comb', name: 'Scissor Over Comb', category: 'Fundamentals' },
  ...
  { id: 'straight-razor', name: 'Straight Razor Shave', category: 'Facial Hair' },
...
export default function SkillsTrackingPage() {
  ...
  const skillsByCategory = BARBER_SKILLS.reduce((acc, skill) => {
  ...
  const studentSkillCounts = skillLogs.reduce((acc, log) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Skills Tracking - Barber Program</h1>
          <p className="text-gray-600 mt-2">Track student progress on specific cuts, fades, and techniques</p>
        </div>

        <div className="grid lg:grid-cols-2 gap-8">
          {/* Left Column - Log New Skill */}
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-bold text-gray-900 mb-6">Log Student Skill</h2>
                  ...
                  <optgroup key={category} label={category}>
                    {skills.map(skill => (
                      <option key={skill.id} value={skill.id}>
                        {skill.name} {studentSkillCounts[skill.id] ? `(${studentSkillCounts[skill.id]} logged)` : ''}
                      </option>
                    ))}
                  </optgroup>
                ))}
              ...
              <>
                ...
                <div className="grid grid-cols-3 gap-4 mb-6">
                  <div className="bg-blue-50 p-4 rounded-lg text-center">
                    <div className="text-2xl font-bold text-blue-600">{skillLogs.length}</div>
                    <div className="text-xs text-gray-600">Total Skills Logged</div>
                  </div>
                  ...
                  <div className="bg-purple-50 p-4 rounded-lg text-center">
                    <div className="text-2xl font-bold text-purple-600">
                      {skillLogs.length > 0 
                        ? (skillLogs.reduce((sum, log) => sum + log.quality_rating, 0) / skillLogs.length).toFixed(1)
                        : '0'}
                    </div>
                    ...
                    <div key={category} className="border-b pb-4">
                      ...
                      <div className="space-y-2">
                        {skills.map(skill => {
                          ...
                          const skillLogs = skillLogs.filter(log => log.skill_id === skill.id);
                          const avgRating = skillLogs.length > 0
                            ? skillLogs.reduce((sum, log) => sum + log.quality_rating, 0) / skillLogs.length
                            : 0;
                          
                          return (
                            <div key={skill.id} className="flex items-center justify-between text-sm">
                              <span className={count > 0 ? 'text-gray-900' : 'text-gray-400'}>
                                {skill.name}
                              </span>
                    ...
                    {skillLogs.slice(0, 10).map(log => (
                      <div key={log.id} className="text-sm bg-gray-50 p-3 rounded">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <div className="flex">
                            {[...Array(5)].map((_, i) => (
                              <svg
                                key={i}
                                className={`w-3 h-3 ${
                                  i < log.quality_rating ? 'text-yellow-400' : 'text-gray-300'
                                }`}
                                fill="currentColor"
                                viewBox="0 0 20 20"
                              >
                          ...
                          {log.notes && (
                            <span className="text-xs text-gray-600">- {log.notes}</span>
                          )}
                        </div>
                      </div>
                    ))}
