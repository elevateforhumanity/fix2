...
const MAX_FILES = parseInt(
  args.get('maxFiles') || process.env.MAX_FILES || '120000',
  10
);
const HASH_LIMIT_BYTES = parseInt(
  args.get('hashLimit') || process.env.HASH_LIMIT || String(512 * 1024),
  10
);
...
const hashFile = (fullPath, size) => {
  ...
  try {
    return hashBuf(fs.readFileSync(fullPath));
  } catch {
    return null;
  }
};
...
const duBytes = (p) => {
  ...
  try {
    const out = execSync(`du -sb "${p}" 2>/dev/null | awk '{print $1}'`, {
      shell: '/bin/bash',
    })
      .toString()
      .trim();
...
const lockFiles = [
  ...
  .map(({ f, p }) => ({
    ...
    sha256: (() => {
      try {
        return hashBuf(fs.readFileSync(p));
      } catch {
        return null;
      }
    })(),
...
const configs = {
  ...
  viteConfigRaw:
    read(path.join(ROOT, 'vite.config.ts')) ??
    read(path.join(ROOT, 'vite.config.js')),
...
function validate(bundle) {
  ...
  if (!nodeOk)
    warn(`Node ${bundle.host.node} detected; recommend 18.x (e.g., 18.20.4).`);

  // Package manager drift
  const locks = (bundle.packageJson.lockFiles || []).map((l) => l.name);
  if (locks.length > 1)
    warn(
      `Multiple lockfiles present: ${locks.join(', ')}. Prefer a single package manager.`
    );
  if (locks.length === 0)
    warn(`No lockfile found. Install to generate one (npm or pnpm).`);

  // Vite presence
  const devDeps = Object.assign(
    {},
    bundle.packageJson.parsed?.devDependencies || {}
  );
  const deps = Object.assign({}, bundle.packageJson.parsed?.dependencies || {});
  const hasVite = Boolean(devDeps.vite || deps.vite);
  if (!hasVite)
    warn(
      `'vite' not found in dependencies. If you build with Vite, add it to devDependencies.`
    );

  // Host conflicts
  if (bundle.configs.netlifyTomlRaw && bundle.configs.vercelJsonRaw) {
    warn(
      'Both Netlify and Vercel configs detected. Use one host to reduce build friction.'
    );
  }
  ...
  if (!bundle.configs.stripeWebhookRaw) {
    warn(
      'Stripe webhook handler not found (netlify/functions/... or api/stripe/webhook...). Add one to enable upgrades.'
    );
  }
  ...
  if (!schema.includes('organizations') || !schema.includes('memberships')) {
    warn(
      'Supabase schema may be missing multitenancy tables (organizations/memberships).'
    );
  }
  if (
    !schema.includes('enable row level security') &&
    !schema.includes('row level security')
  ) {
    warn(
      'Supabase RLS not detected in schema.sql (ensure RLS is enabled for org-scoped tables).'
    );
  }
  ...
  if (
    ...
    exists(path.join(ROOT, '.env.local'))
  ) {
    warn(
      "Found .env in repo root. Ensure it's .gitignored and not committed; rotate any leaked keys."
    );
  }

  // Sitemap/robots
  if (!bundle.configs.robotsTxtRaw)
    warn(
      'robots.txt not found in /public. Add one that points to sitemap index.'
    );
  if (!bundle.configs.sitemapIndexRaw)
    warn(
      'sitemap.xml index not found in /public. Generate index + child sitemaps.'
    );

  // Pages coverage
  if (bundle.pages.truncated)
    warn(
      `Pages listing truncated at ${MAX_FILES}. Increase maxFiles= if needed.`
    );
...
console.log(
  `   Sizes: workspace=${bundle.sizes.human.workspace}, node_modules=${bundle.sizes.human.node_modules}, dist=${bundle.sizes.human.dist}`
);
console.log(
  `   Host: node=${bundle.host.node}, npm=${bundle.host.npm}, os=${bundle.host.os}\n`
);
for (const f of findings) {
  const tag =
    f.level === 'error' ? 'ERROR' : f.level === 'warn' ? 'WARN' : 'OK';
...
export default function FundingImpact() {
  return (
    <main style={{ maxWidth: 900, margin: '2rem auto', padding: '0 1rem' }}>
      ...
      <p>
        Elevate for Humanity + Selfish Inc. align workforce training with public
