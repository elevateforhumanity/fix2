...
// Enrollment injector script - this is what we're adding
const ENROLLMENT_SCRIPT_URL =
  'https://main--elevateforhumanityfix.netlify.app/enrollment-injector.js';

const ENROLLMENT_SCRIPT = `<script src="${ENROLLMENT_SCRIPT_URL}" defer></script>`;
...
async function integrateDurableBridge() {
  ...
  try {
    ...
    // Step 5: Add enrollment script to head
    console.log('🔧 Step 5: Adding enrollment script to head section...');
    ...
    for (const selector of headCodeSelectors) {
      try {
        ...
        if (existingCode.includes(ENROLLMENT_SCRIPT_URL)) {
          console.log('✅ Enrollment script already exists in head section');
          headCodeFound = true;
          break;
        }
        ...
        console.log('✅ Added enrollment script to head section');
        headCodeFound = true;
        break;
      } catch (e) {
        continue;
    ...
    if (!headCodeFound) {
      console.log('⚠️  Could not add enrollment script automatically');
    ...
    const enrollmentLoaded = await page.evaluate((url) => {
      const scripts = Array.from(document.querySelectorAll('script'));
      return scripts.some((script) => script.src.includes(url));
    }, ENROLLMENT_SCRIPT_URL);

    if (enrollmentLoaded) {
      console.log('✅ Enrollment script is loaded on site');
      success = true;
    } else {
      console.log('⚠️  Enrollment script not detected on site');
  ...
  if (success) {
    ...
    console.log('');
    console.log('Enrollment script is now integrated with Durable.co!');
    console.log('');
    console.log('Next steps:');
    console.log('1. Visit www.elevateforhumanity.org');
    console.log('2. Verify enrollment programs appear on site');
    ...
    console.log('3. Add enrollment script to head section:');
    console.log('');
    console.log(ENROLLMENT_SCRIPT);
  }
  console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  console.log('');
  ...
  const status = {
    ...
    success: success,
    enrollment_script_url: ENROLLMENT_SCRIPT_URL,
