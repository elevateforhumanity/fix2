...
function readLocalSitemap() {
  try {
    ...
    const urls = locMatches
      ? locMatches.slice(0, 5).map((m) => m.replace(/<\/?loc>/g, ''))
      : [];
...
async function main() {
  ...
  log(
    '    4. Enter full URL: https://elevateforhumanity.org/sitemap.xml',
    'reset'
  );
  log('    5. Click Submit', 'reset');
  ...
  const primaryDomain = results.find(
    (r) => r.domain.includes('.org') && !r.domain.includes('www')
  );
  if (primaryDomain && primaryDomain.sitemap.success) {
    ...
    log(
      '\n  Note: Pinging notifies search engines but manual submission is recommended.',
      'yellow'
    );
  }
  ...
  log(
    `  Domains accessible: ${accessible}/${total}`,
    accessible === total ? 'green' : 'yellow'
  );
  ...
  if (accessible === total) {
    ...
    log(
      '\n  ⚠️  Some domains are not accessible. Check deployment status.',
      'yellow'
    );
  }
