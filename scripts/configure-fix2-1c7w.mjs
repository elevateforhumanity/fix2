...
async function main() {
  ...
  const projects = await fetch(`https://api.vercel.com/v9/projects?teamId=${teamId}`, {
    headers: { 'Authorization': `Bearer ${VERCEL_TOKEN}` }
  }).then(r => r.json());
  ...
  // Trigger deployment
  const deployment = await fetch(`https://api.vercel.com/v13/deployments?teamId=${teamId}`, {
    method: 'POST',
    headers: {
      'Authorization': `Bearer ${VERCEL_TOKEN}`,
      'Content-Type': 'application/json'
  ...
  console.log('   Check status at: https://vercel.com/gitpod/fix2-1c7w/deployments\n');
}
