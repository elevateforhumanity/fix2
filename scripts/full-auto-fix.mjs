...
try {
  ...
  if (response.ok) {
    ...
    console.log(`   ✅ Token valid for user: ${user.user?.username || user.user?.email || 'unknown'}`);
  } else {
...
try {
  ...
  execSync(`git commit -m "🤖 Auto-fix: Update build marker and trigger deployment

Co-authored-by: Ona <no-reply@ona.com>"`, { stdio: 'inherit' });
...
console.log('  2. Check: https://vercel.com/elevate-48e460c9/fix2-gpql/deployments');
