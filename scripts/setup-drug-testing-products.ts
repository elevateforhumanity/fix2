...
async function createProduct(item: any, type: 'service' | 'course') {
  try {
    ...
    console.log(`✅ Created: ${item.name} - $${item.price} (Product: ${product.id}, Price: ${price.id})`);
...
async function main() {
  console.log('🚀 Setting up Drug Testing Products and Courses in Stripe...\n');
  
  // Create Drug Testing Services
  console.log('📦 Creating Drug Testing Services (20 products)...\n');
  ...
  console.log(`\n✅ Created ${servicesCreated}/${drugTestingServices.length} drug testing services\n`);
  
  // Create Training Courses
  console.log('🎓 Creating Training Courses (12 courses)...\n');
  ...
  console.log(`\n✅ Created ${coursesCreated}/${trainingCourses.length} training courses\n`);
