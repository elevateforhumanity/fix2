...
const GITHUB_USERNAME =
  ...
  process.env.NETLIFY_EMAIL ||
  'YOUR_GITHUB_USERNAME';
const GITHUB_PASSWORD =
  process.env.GITHUB_PASSWORD ||
  process.env.NETLIFY_PASSWORD ||
  ...
  try {
    ...
    if (githubButton) {
      ...
      if (currentUrl.includes('github.com/login')) {
        ...
        if (has2FA) {
          console.log(
            '   ⚠️  2FA detected - you need to enter the code manually'
          );
    ...
    for (let i = 0; i < VARIABLES.length; i++) {
      ...
      console.log(
        `   [${i + 1}/${VARIABLES.length}] Adding ${variable.key}...`
      );
      ...
      try {
        ...
        // Fill in key
        const keyInput = await page.waitForSelector(
          'input[name="key"], input[placeholder*="Key"]',
          { timeout: 5000 }
        );
        ...
        // Fill in value
        const valueInput = await page.waitForSelector(
          'input[name="value"], textarea[name="value"], input[placeholder*="Value"]',
          { timeout: 5000 }
        );
        ...
        if (variable.scopes.includes('Production')) {
          ...
          const scopeSelector = await page.$(
            'select[name="scope"], select[name="context"]'
          );
          if (scopeSelector) {
...
if (require.main === module) {
  if (
    ...
    console.error(
      '❌ Error: Please set GITHUB_USERNAME and GITHUB_PASSWORD environment variables\n'
    );
    console.error('Usage:');
    console.error(
      '  GITHUB_USERNAME=youruser GITHUB_PASSWORD=yourpass node scripts/puppeteer-add-netlify-vars.js\n'
    );
    console.error('Or use existing browser session:');
    console.error(
      '  USE_EXISTING_SESSION=true node scripts/puppeteer-add-netlify-vars.js\n'
    );
    process.exit(1);
  }
