...
async function fetchUrl(url, timeout = 15000) {
  return new Promise((resolve, reject) => {
    ...
    https
      .get(
        url,
        { headers: { 'User-Agent': 'Mozilla/5.0 (deployment verification)' } },
        (res) => {
          clearTimeout(timer);
          let data = '';
  ...
  for (const url of urls) {
    try {
      ...
      if (response.statusCode === 200) {
        ...
        const checks = [
          {
            name: 'Full content title',
            test: content.includes('Launch Your AI'),
          },
          {
            name: 'Comprehensive description',
            test: content.includes('federally-funded'),
          },
          {
            name: 'Navigation present',
            test:
              content.includes('efh-nav-primary') ||
              content.includes('Programs'),
          },
          { name: 'Substantial content', test: content.length > 10000 },
          {
            name: 'Proper domain in meta',
            test: content.includes('elevateforhumanity.org'),
          },
...
async function main() {
  ...
  while (attempts < maxAttempts) {
    ...
    if (attempts < maxAttempts) {
      console.log(
        '⚠️  Full site not yet live, waiting 45 seconds before next check...'
      );
