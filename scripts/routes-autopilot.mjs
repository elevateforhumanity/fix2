...
const NOT_FOUND_VIRTUAL = path.join(
  SRC_DIR,
  'pages/__generated__',
  'NotFound.jsx'
);
...
async function generateRouter(entries, prioritized) {
  ...
  const lazyName = (file) => {
    ...
    return (
      ...
      id +
      '_' +
      crypto.createHash('md5').update(file).digest('hex').slice(0, 6)
    );
  };
  ...
  for (const { route, file } of entries) {
    ...
    imports.push(
      `const ${symbol} = React.lazy(() => import(/* @vite-ignore */ '${rel}'));`
    );
  ...
  // Build routes array
  const routeLines = ordered.map(
    ({ route, symbol }) => `  { path: '${route}', element: <${symbol} /> },`
  );
...
async function scanComponents() {
  ...
  // Map to routes
  const pairs = files
    .map((f) => ({ file: f, route: routePathFromFile(f) }))
  ...
  for (const p of pairs) {
    ...
    else if (prev.file.includes('/components/') && p.file.includes('/pages/'))
      seen.set(key, p);
  }
...
async function writeReport(stats) {
  ...
  lines.push(
    `- Coverage vs. config: **${((stats.generatedCount / Math.max(1, stats.configCount)) * 100).toFixed(1)}%**`
  );
  lines.push(``);
  ...
  if (stats.missingFromComponents.length) {
    ...
    for (const r of stats.missingFromComponents.slice(0, 300))
      lines.push(`- ${r}`);
  }
...
async function main() {
  ...
  if (missingFromComponents.length) {
    console.log(
      `⚠️  ${missingFromComponents.length} routes missing components`
    );
  }
  ...
  await fs.writeFile(
    SITEMAP_FILE,
    buildSitemapXml(entries.map((e) => e.route))
  );

  // Report
  const extras = [...discoveredMap.keys()].filter(
    (r) => !allRoutes.includes(r)
  );
  await writeReport({
