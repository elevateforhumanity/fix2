#!/usr/bin/env bash
# == bootstrap_claimbot.sh ==
# Usage:
#   bash bootstrap_claimbot.sh
# Then:
#   python -m venv .venv && source .venv/bin/activate
#   pip install -r requirements.txt && python -m playwright install
#   python claimbot.py --all    # or: python claimbot.py --only yelp
...
{
  "name": "EFH ClaimBot (Codespaces)",
  "image": "mcr.microsoft.com/devcontainers/python:3.11",
  "features": { "ghcr.io/devcontainers/features/node:1": {} },
  "postCreateCommand": "pip install -r requirements.txt && python -m playwright install chromium",
  "customizations": {
    "vscode": {
      "extensions": ["ms-python.python", "ms-playwright.playwright"],
...
cat > README.md <<'MD'
# EFH ClaimBot — Directory Claim Assistant

This assistant opens/helps pre-fill claim or submit pages for:
...
python claimbot.py --photos     # print photo + document checklist
```

Edit `config.yaml` for your org info. Toggle targets in `directories.csv` (Enabled=true/false).
MD
...
def main():
    parser = argparse.ArgumentParser(description="EFH Directory Claim Assistant")
    ...
    with sync_playwright() as p:
        ...
        for _, row in df.iterrows():
            ...
            categories = CATEGORY_HINTS.get(key, [])
            print(f"\n[OPENED] {row['Platform/Directory']} → {url}")
            print(f"  Notes: {notes}")
            if categories:
            ...
            print("  → Complete the on-page steps, then press ENTER here to continue to the next site.")
            try:
                input()
            except KeyboardInterrupt:
                break

        print("\nAll targets processed. Close the browser if it is still open.")
        browser.close()
...
4. python claimbot.py --all     # or: python claimbot.py --only yelp
5. python claimbot.py --photos  # optional photo & category checklist
OUTRO
