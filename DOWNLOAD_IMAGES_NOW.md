# 📸 Download Images Now - Quick Guide

## 🚀 Fastest Way: Use the Interactive Guide

**Visit**: [https://3000--019a8014-910c-7bbe-89d1-9e340ed3ede3.us-east-1-01.gitpod.dev/download-images.html](https://3000--019a8014-910c-7bbe-89d1-9e340ed3ede3.us-east-1-01.gitpod.dev/download-images.html)

This page has:

- ✅ Direct links to Pexels for each image
- ✅ Exact filenames to use
- ✅ Step-by-step instructions
- ✅ Priority indicators
- ✅ Visual layout

**Time**: 30-45 minutes to download all images

---

## 📋 Quick Checklist

### Priority 1 (Do First):

- [ ] Hero Banner (1 image) - `efh-hero.jpg`
- [ ] HVAC (2 images) - card + hero
- [ ] Welding (2 images) - card + hero
- [ ] CDL (2 images) - card + hero

### Priority 2:

- [ ] Medical Assistant (2 images)
- [ ] Nail Tech (2 images)

**Total**: 11 images needed

---

## 🎯 Step-by-Step Process

### 1. Open the Download Guide

Visit: `/download-images.html` on your dev server

### 2. For Each Image:

1. Click "View on Pexels" button
2. Browse and select a good image
3. Click green "Free Download" button
4. Choose "Original" or "Large" size
5. Save with exact filename shown
6. Place in correct folder

### 3. Compress Images

- Visit: https://tinypng.com
- Upload all downloaded images
- Download compressed versions
- Replace originals

### 4. Test on Site

- Refresh homepage
- Check program pages
- Verify all images load

---

## 📁 Where to Save Images

```
public/
├── hero/
│   └── efh-hero.jpg          ← Hero banner
└── images/
    ├── efh-hvac-card.jpg     ← HVAC card
    ├── efh-hvac-hero.jpg     ← HVAC hero
    ├── efh-welding-card.jpg  ← Welding card
    ├── efh-welding-hero.jpg  ← Welding hero
    ├── efh-cdl-card.jpg      ← CDL card
    ├── efh-cdl-hero.jpg      ← CDL hero
    ├── efh-medical-assistant-card.jpg
    ├── efh-medical-assistant-hero.jpg
    ├── efh-nail-tech-card.jpg
    └── efh-nail-tech-hero.jpg
```

---

## 🔍 What to Look For

### Hero Banner:

- Diverse group of students
- Training or classroom setting
- Professional, well-lit
- Smiling, engaged people
- Modern facility

### Program Images:

- **HVAC**: Technician with AC unit, tools, professional setting
- **Welding**: Welder with mask, sparks, safety gear
- **CDL**: Truck driver, semi truck, highway
- **Medical**: Healthcare worker, medical setting, professional
- **Nail Tech**: Manicurist, nail salon, professional service

---

## ⚡ Even Faster: Use These Direct Searches

### Pexels Search Links:

1. **Hero**: https://www.pexels.com/search/diverse%20students%20training/
2. **HVAC**: https://www.pexels.com/search/hvac%20technician/
3. **Welding**: https://www.pexels.com/search/welding/
4. **CDL**: https://www.pexels.com/search/truck%20driver/
5. **Medical**: https://www.pexels.com/search/medical%20assistant/
6. **Nail Tech**: https://www.pexels.com/search/nail%20technician/

---

## ✅ After Downloading

### Verify:

- [ ] All filenames match exactly (case-sensitive)
- [ ] Images are in correct folders
- [ ] File sizes are reasonable (<500KB each)
- [ ] Images look professional
- [ ] No watermarks or logos

### Test:

- [ ] Homepage hero displays
- [ ] Program cards show images
- [ ] Images load quickly
- [ ] Mobile view looks good

---

## 🆘 Troubleshooting

**Image won't download?**

- Try a different browser
- Right-click > Save Image As
- Check Pexels is accessible

**Wrong size?**

- Download "Original" or "Large"
- Resize if needed at: https://www.iloveimg.com/resize-image

**File too large?**

- Compress at: https://tinypng.com
- Target: <200KB for program images

---

## 📊 Progress Tracker

Track your progress:

```
Hero Banner:     [ ]
HVAC Card:       [ ]
HVAC Hero:       [ ]
Welding Card:    [ ]
Welding Hero:    [ ]
CDL Card:        [ ]
CDL Hero:        [ ]
Medical Card:    [ ]
Medical Hero:    [ ]
Nail Tech Card:  [ ]
Nail Tech Hero:  [ ]
```

---

## 🎉 When Complete

Your site will have:

- ✅ Professional hero banner
- ✅ All program images
- ✅ No more placeholders
- ✅ Ready for marketing

**Estimated Time**: 30-45 minutes
**Difficulty**: Easy
**Cost**: Free

---

## 🔗 Quick Links

- **Interactive Guide**: `/download-images.html`
- **Pexels**: https://www.pexels.com
- **TinyPNG**: https://tinypng.com
- **Image Resize**: https://www.iloveimg.com/resize-image

---

**Start Now**: Open `/download-images.html` and begin downloading! 🚀
