...
-- User Streaks Table (if not exists)
...
-- xAPI Statements Table (if not exists)
...
-- Program Holder Notes Table (if not exists)
...
-- AI Generated Courses Table (if not exists)
...
CREATE POLICY "Admins can view all applications"
  ...
  USING (
    EXISTS (
      SELECT 1 FROM user_profiles
      WHERE user_id = auth.uid() AND role IN ('admin', 'delegate')
    )
  );
