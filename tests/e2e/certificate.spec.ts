...
test.describe('Certificate Generation', () => {
  ...
  test('should show earned certificates', async ({ page }) => {
    ...
    const hasCertificates = await page.locator('.certificate-card, [data-testid="certificate"]').count();
    const noMessage = await page.locator('text=/no certificates|complete.*course/i').count();
  ...
  test('should generate certificate after course completion', async ({ page }) => {
    ...
    await expect(page.locator('.certificate-card, [data-testid="certificate"]')).toBeVisible();
  });

  test('should display certificate details', async ({ page }) => {
    await page.goto('/certificates');
    
    const certificate = page.locator('.certificate-card, [data-testid="certificate"]').first();
  ...
  test('should share certificate', async ({ page }) => {
    ...
    const shareButton = page.locator('button:has-text("Share"), a:has-text("Share")').first();
    
    if (await shareButton.count() > 0) {
      await shareButton.click();
      
      // Should show share options
      await expect(page.locator('text=/linkedin|twitter|facebook|copy link/i')).toBeVisible();
    }
  });
  ...
  test('should verify certificate authenticity', async ({ page }) => {
    ...
    await expect(page.locator('input[name="certificateId"], input[placeholder*="certificate"]')).toBeVisible();
  });
  ...
  test('should validate certificate ID', async ({ page }) => {
    ...
    await expect(page.locator('text=/valid|invalid|verified|not found/i')).toBeVisible();
  });
});
...
test.describe('Certificate Display', () => {
  ...
  test('should render certificate preview', async ({ page }) => {
    ...
    if (await certificate.count() > 0) {
      ...
      await expect(page.locator('img[alt*="certificate"], canvas')).toBeVisible();
    }
  });
  ...
  test('should display certificate metadata', async ({ page }) => {
    ...
    if (await certificate.count() > 0) {
      ...
      await expect(certificate.locator('text=/\\d{4}|\\d{1,2}\\/\\d{1,2}/i')).toBeVisible();
    }
  });
