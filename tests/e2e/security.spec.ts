...
test.describe('XSS Protection', () => {
  test('should sanitize user input in forms', async ({ page }) => {
    ...
    await page.fill('input[name="name"], input[type="text"]:first-child', xssPayload);
  ...
  test('should sanitize HTML in rendered content', async ({ page }) => {
    ...
    const scripts = await page.locator('script[src*="javascript:"], script:has-text("alert")').count();
    expect(scripts).toBe(0);
    
    // Check that no event handlers in HTML
    const eventHandlers = await page.locator('[onclick], [onerror], [onload]').count();
...
test.describe('Authentication Security', () => {
  test('should have secure password requirements', async ({ page }) => {
    ...
    await expect(page.locator('text=/password.*weak|password.*short|password.*characters/i')).toBeVisible();
  });
...
test.describe('SQL Injection Protection', () => {
  test('should sanitize database queries', async ({ page }) => {
    ...
    await expect(page.locator('text=/sql|syntax error|database/i')).not.toBeVisible();
  });
});
