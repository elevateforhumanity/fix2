...
test.describe('Admin Dashboard Access', () => {
  ...
  test('should allow admin access with proper role', async ({
    page,
    context,
  }) => {
...
test.describe('Course Management', () => {
  ...
  test('should create new course', async ({ page }) => {
    ...
    // Click create button
    await page.click(
      'button:has-text("Create"), button:has-text("New"), a:has-text("Add")'
    );

    // Should show course form
    await expect(
      page.locator('input[name="title"], input[name="name"]')
    ).toBeVisible();
  });
...
test.describe('Analytics Dashboard', () => {
  ...
  test('should display analytics overview', async ({ page }) => {
    ...
    // Should show metrics
    await expect(
      page.locator('text=/users|enrollments|revenue/i')
    ).toBeVisible();
  });
...
test.describe('Autopilot Admin', () => {
  ...
  test('should show automation suites', async ({ page }) => {
    ...
    await expect(
      page.locator('text=/suite|workflow|automation/i')
    ).toBeVisible();
  });
  ...
  test('should trigger automation', async ({ page }) => {
    ...
    if ((await runButton.count()) > 0) {
      ...
      await expect(
        page.locator('text=/running|executing|processing/i')
      ).toBeVisible();
    }
  });
});
