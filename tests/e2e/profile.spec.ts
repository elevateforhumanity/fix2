...
test.describe('User Profile Management', () => {
  ...
  test('should upload profile picture', async ({ page }) => {
    ...
    if (await fileInput.count() > 0) {
      ...
      await expect(page.locator('img[alt*="profile"], img[alt*="avatar"]')).toBeVisible();
    }
  });
});
...
test.describe('Password Management', () => {
  ...
  test('should require current password', async ({ page }) => {
    ...
    // Should show error
    await expect(page.locator('text=/current password|old password/i')).toBeVisible();
  });
...
test.describe('Account Deletion', () => {
  ...
  test('should show delete account option', async ({ page }) => {
    ...
    const deleteButton = page.locator('button:has-text("Delete"), button:has-text("Close Account")');
  ...
  test('should require confirmation for deletion', async ({ page }) => {
    ...
    if (await deleteButton.count() > 0) {
      ...
      await expect(page.locator('text=/confirm|are you sure|permanent/i')).toBeVisible();
    }
  });
});
